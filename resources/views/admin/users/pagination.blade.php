<?php $link_limit = 11; ?>
    ...
    <ul class="pagination">
        @if(($pagination->current_page - 1) != null)
            <li>
                <a href="{{ route('admin.users',array_merge($prev_search, ['page'=>($pagination->current_page - 1)])) }}">&laquo;</a>
            </li>
        @else
        ...
        @for($i = 1; $i <= $pagination->last_page; $i++)
            ...
            @if($from < $i && $i < $to)
                <li class="{{ $pagination->current_page == $i ? 'active' : false}}">
                    <a href="{{route('admin.users',array_merge($prev_search, ['page'=>$i]))}}">{{$i}}</a>
                </li>
            @endif
        @endfor
        
        @if(($pagination->current_page + 1) <= $pagination->last_page)
            <li><a href="{{ route('admin.users',array_merge($prev_search, ['page'=>($pagination->current_page + 1)]) ) }}">&raquo;</a></li>
        @else
